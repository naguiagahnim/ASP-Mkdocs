...
##  Résumé des nouveaux fichiers et composants

### Nouveaux fichiers créés depuis le document BM25

| Fichier | Rôle |
|---------|------|
| `cli/src/main.rs` | CLI principal avec 3 modes |
| `cli/src/mistral.rs` | Client OCR Mistral via LiteLLM |
| `cli/src/process.rs` | Téléchargement et conversion PDF→PNG |
| `cli/src/document_processor.rs` | Structures `DocumentEntry`, `UICoordinates`, etc. |
| `cli/base.json` | Schéma JSON Schema pour l'OCR Mistral |
| `chatbot/src/api_interrogator.rs` | Client LLM pour reformulation et synthèse |
| `chatbot/src/domain/finder.rs` | Trait `Finder` (interface) |
| `chatbot/src/domain/bm25_finder.rs` | Moteur hybride BM25+LLM (évolution majeure) |

### Fichiers modifiés significativement

| Fichier | Modifications |
|---------|---------------|
| `chatbot/src/domain/structs.rs` | Ajout de `breadcrumb_trail`, `context_prerequisites` dans `KnowledgeEntry` |
| `chatbot/Cargo.toml` | Nouvelles dépendances : `reqwest`, `tokio`, `serde_json` |
...
```bash
OPENAI_BASE_URL=https://litellm-proxy-url
OPENAI_API_KEY=your-api-key
OPENAI_OCR_MODEL=mistral/mistral-ocr-latest
```
...
OPENAI_BASE_URL=https://litellm-proxy-url
OPENAI_API_KEY=your-api-key
OPENAI_CHAT_MODEL=mistral/mistral-small-latest
DATA_DIR=/path/to/data  # Optionnel, fallback sur ./data ou ../data
```

---

Ce document capture l'état complet et détaillé du projet après toutes les évolutions majeures décrites.
